...
def validate_deployment_success(event):
    ...
    return {'success': True, 'reason': 'All tests passed'}
        ...
        codedeploy_hook_resource_arn = (
            f"arn:{Aws.PARTITION}:codedeploy:{Aws.REGION}:{Aws.ACCOUNT_ID}:deploymentgroup:"
            f"{self.codedeploy_ecs_app.application_name}/{self.codedeploy_ecs_deployment_group.deployment_group_name}"
        )

        # Grant permissions for hooks to interact with CodeDeploy (restricted resource)
        self.pre_deployment_hook.add_to_role_policy(
            iam.PolicyStatement(
                effect=iam.Effect.ALLOW,
                actions=["codedeploy:PutLifecycleEventHookExecutionStatus"],
                resources=[codedeploy_hook_resource_arn],
            )
        )
