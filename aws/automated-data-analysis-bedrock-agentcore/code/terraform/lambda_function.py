...
export class BusinessContinuityTestingStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: BusinessContinuityTestingStackProps = {}) {
    ...
    const lambdaFunctions = this.createLambdaFunctions(projectId, this.automationRole, this.testResultsBucket, snsTopic, commonTags);
  ...
  private createAutomationRole(projectId: string, tags: Record<string, string>): iam.Role {
    ...
    role.addToPolicy(
      new iam.PolicyStatement({
        ...
        actions: ['ssm:*', 'ec2:*', 'rds:*', 's3:*', 'lambda:*', 'states:*', 'events:*', 'cloudwatch:*', 'sns:*', 'logs:*', 'backup:*'],
        resources: ['*']
      })
    );
   ...
   */
  private createSnsNotificationTopic(projectId: string, notificationEmail?: string, tags: Record<string, string> = {}): sns.Topic {
...
def execute_application_test(ssm):
   ...
   */
  private createTestingSchedules(projectId: string, orchestratorFunction: lambda.Function, tags: Record<string, string>): void {
    // Daily basic tests
    ...
    dailyRule.addTarget(new targets.LambdaFunction(orchestratorFunction, { event: events.RuleTargetInput.fromObject({ testType: 'daily' }) }));
    ...
    weeklyRule.addTarget(new targets.LambdaFunction(orchestratorFunction, { event: events.RuleTargetInput.fromObject({ testType: 'weekly' }) }));
    ...
    monthlyRule.addTarget(new targets.LambdaFunction(orchestratorFunction, { event: events.RuleTargetInput.fromObject({ testType: 'monthly' }) }));
      ...
      widgets: [
        [
          ...
          new cloudwatch.GraphWidget({
            ...
            left: [lambdaFunctions.orchestrator.metricDuration(), lambdaFunctions.orchestrator.metricErrors(), lambdaFunctions.orchestrator.metricInvocations()],
            width: 12,
            height: 6
          }),
        ...
        [
          ...
          new cloudwatch.LogQueryWidget({
            ...
            logGroups: [logs.LogGroup.fromLogGroupName(this, 'OrchestratorLogGroup', `/aws/lambda/${lambdaFunctions.orchestrator.functionName}`)],
