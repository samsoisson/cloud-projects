...
def check_application_health(events, context):
  ...
  private createLambdaFunctions(
    ...
    const expectedBucketOwner = cdk.Stack.of(this).account;
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Store results in S3 with bucket owner verification
        s3.put_object(
            Bucket=os.environ['RESULTS_BUCKET'],
            Key=f'test-results/{test_type}/{test_id}/results.json',
            Body=json.dumps(test_results, indent=2),
            ContentType='application/json',
            ExpectedBucketOwner=os.environ.get('EXPECTED_BUCKET_OWNER')
        )
