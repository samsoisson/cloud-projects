...
class QDeveloperInfrastructureStack(Stack):
    ...
    def _create_lambda_execution_role(self, suffix: str) -> iam.Role:
        ...
        # Restrict CloudFormation permissions to avoid privilege escalation via "*" resources.
        # Only allow template validation globally; scope stack operations to stacks created by this automation.
        role.add_to_policy(
            iam.PolicyStatement(
                effect=iam.Effect.ALLOW,
                actions=[
        ...
        role.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=[
                    f"arn:{cdk.Aws.PARTITION}:cloudformation:{cdk.Aws.REGION}:{cdk.Aws.ACCOUNT_ID}:stack/q-developer-stack-*/*"
                ],
            )
        )
